 ...
 *
 * Fetches puzzle data from the NYT Connections API and merges it into
 * our local data file.
 *
 * Run manually:   node scripts/update-connections.mjs
 * Backfill:       node scripts/update-connections.mjs --backfill
...
// NYT Connections API — publicly accessible, no auth needed
...
/**
 * Validate a Connections puzzle has the expected fields
 */
function isValidPuzzle(puzzle) {
  ...
  for (const cat of puzzle.categories) {
    if (!cat.title || !Array.isArray(cat.cards) || cat.cards.length !== 4)
      return false;
  }
  return true;
}
...
async function fetchPuzzle(dateStr) {
  const data = await fetchJSON(`${NYT_CONNECTIONS_API}/${dateStr}.json`, 2);
  if (!data) return null;
